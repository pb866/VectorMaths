...
function vector_maths(vectors::DataFrames.DataFrame...;
  ...
  if output != "offset" && output != "scale"
    throw(ArgumentError(string("Wrong keyword argument!\n",
      "\33[0mUse \"offset\" or \"scale\" for `output` or assign a `Vector{Symbol}`.")))
  end
...
function compile_data(vectors, xcols, ycols, colnames)
  ...
  for i = 1:length(vectors)
    ...
    if xcols[i] isa Vector && length(xcols[i]) ≠ length(ycols[i])
      throw(ColError("Different number of `xcols` and `ycols` column definitions.",
        string("Use `Int` for `xcols` and `Int` or `Vector{Int}` for `ycols` ",
          "or use `Vector{Int}` of same length for both column types.")))
    end
    ...
    elseif ycols[i] isa Vector
      push!(xdata, [vectors[i][xcols[i]] for j = 1:length(ycols[i])]...)
      ydata = assign_ydata(ycols[i], ydata, vectors[i])
    else
...
function get_headers(colnames, xcols, ycols)
  ...
  if colnames isa Vector
    length(colnames) != length(y) + 3 ? throw(ColError()) : return colnames
  end
  if colnames == "default"
...
function test_monotonicity(xdata)
  ...
  for (i, x) in enumerate(xdata)
    if x ≠ sort(unique(x))
      throw(DataError("x data in DataFrame $i not strictly monotonic", x))
    end
  end
...
function unify_xdata(xdata)
  ...
  if maximum(mins) > minimum(maxs)
    throw(DataError("No common x data range."))
  end
